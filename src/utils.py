...
def get_net_info(num_classes):
    net = torch.nn.parallel.DataParallel(models.ResNet50()).cuda()
    head = torch.nn.parallel.DataParallel(models.Head()).cuda()
    # Предположим, что Head выдает тензор размера (batch_size, 256)
...
def load_net(args, net, head, classifier):
    ...
    if os.path.exists(save_folder + '/net.pt'):
        net.module.load_state_dict(torch.load(save_folder + '/net.pt'), strict=False)
    else:
        print("Warning: net.pt not found, using default pretrained model.")

    if os.path.exists(save_folder + '/head.pt'):
        head.module.load_state_dict(torch.load(save_folder + '/head.pt'), strict=False)
    else:
        print("Warning: head.pt not found, using initialized head.")

    if os.path.exists(save_folder + '/classifier.pt'):
        classifier.module.load_state_dict(torch.load(save_folder + '/classifier.pt'), strict=False)
    else:
        print("Warning: classifier.pt not found, using initialized classifier.")
